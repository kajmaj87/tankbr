...
p.add(
    ...
    "--rounds",
    type=int,
    help="how many full rounds to generate. round means (players/match_size) matches",
)
p.add(
    "-m",
    "--match_size",
    type=int,
    help="how many players should take part in one match",
)
p.add(
    "-s",
    "--matching_spread",
...
p.add(
    "--include_human_player",
    action="store_true",
    help="true to allow human player to play",
)
...
p.add(
    ...
    "--gen_min_sigma",
    type=float,
    help="minimum rank certainty before being kicked out of tournament",
)
p.add(
    "-q",
    "--gen_worst_quantile",
...
p.add(
    "--gui_zoom_change_factor",
    type=float,
    help="determines how fast zoom changes when scrolling",
)
p.add(
    "--gui_offset_x",
    type=float,
    help="how much viewport should be offset horizontally when starting the game",
)
p.add(
    "--gui_offset_y",
    type=float,
    help="how much viewport should be offset vertically when starting the game",
)
...
p.add(
    "--game_spawn_range",
    type=int,
    help="players will be spawn in a square of 2*spawn_range around (0,0)",
)
p.add(
    "--game_movement_speed",
    type=int,
    help="what is the max speed that tanks move per game turn",
)
p.add(
    "--game_rotation_speed",
    type=int,
    help="what is the max speed of rotation (turret and body) per game turn",
)
p.add("--game_gun_load_time", type=int, help="how many turns it takes to load a gun")
p.add(
    "--game_ammo",
    type=int,
    help="how much ammo in reserve every player has (all players start with a loaded gun)",
)
p.add("--game_bullet_speed", type=int, help="how fast a turn each bullet goes")
p.add(
    "--game_bullet_position_offset",
    type=int,
    help="how far from player does the bullet spawn - needed ",
)
