...
def create_model(layers, seed=1, random_scale=0.1):
    ...
    for i in range(1, len(layers)):
        architecture.append({"input_dim": layers[i-1], "output_dim": layers[i], "activation": "relu" if i-1<len(layers) else "sigmoid"})
...
def init_layers(nn_architecture, seed=1, random_scale=0.1):
    ...
    for i, layer in enumerate(nn_architecture):
        ...
        params_values["W" + str(i)] = (
            np.random.randn(layer_output_size, layer_input_size) * random_scale
        )
        params_values["b" + str(i)] = (
            np.random.randn(layer_output_size, 1) * random_scale
        )
...
def single_layer_forward_propagation(
    previous_activation, weights, bias, activation="relu"
):
...
def full_forward_propagation(x, params_values, nn_architecture):
    ...
    for i, layer in enumerate(nn_architecture):
        ...
        current_activation = single_layer_forward_propagation(
            previous_activation, weights, bias, activation_function
        )
